...
from vantage_ai_backend.date_parser.date_parser import combine_datetime_columns
...
def on_folder_submit(abs_folderpath):
    ...
    if len(err_str) > 0:
        ...
        for file in os.listdir(abs_folderpath):
            ...
            #check that abs_path is an existing regular file and file is within size limit of 200MB
            if os.path.isfile(abs_path) and (os.path.getsize(abs_path) / 10**6 <= 200):
