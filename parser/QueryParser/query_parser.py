...
"""
The following code parses the query without taking the column names from the df
"""
...
class QueryParser:
    # Initializing QueryParser
    def __init__(self, dataframe):
        self.df = dataframe
        self.column_names = {col.lower():col for col in dataframe.columns}
        self.columns = [col.lower() for col in dataframe.columns]
    
    # Parsing query, main function ran by QueryParser
    def parse_query(self, query):
        doc = nlp(query.lower())
    ...
    def extract_target_columns(self, doc):
        ...
        for token in doc:
            if token.text in self.columns:
                columns.add(self.column_names[token.text])
            elif token.lemma_ in self.columns:
                columns.add(self.column_names[token.lemma_])
    ...
    def extract_filters(self, doc):
        ...
        while i < len(tokens):
            if tokens[i].text in self.columns:
                ...
                column = self.column_names[tokens[i].text]
                i += 1
            ...
            if token.text in group_by_words:
                if i + 1 < len(tokens):
                    ...
                    if next_token.text in self.columns:
                        group_by_columns.append(self.column_names[next_token.text])
                ...
                if word in query_text:
                    ...
                    for token in doc:
                        if token.text in self.columns or token.lemma_ in self.columns:
                            return {
                                "function": function,
                                "column": self.column_names[token.text]
                            }
        ...
        for i, token in enumerate(tokens):
            if token.lemma_ in ['sort', 'order', 'highest', 'largest', 'smallest', 'lowest', 'descending', 'ascending']:
                ...
                if i + 1 < len(tokens):
                    next_token = tokens[i + 1]
                    if next_token.text in self.columns:
                        sort_columns.append(self.column_names[next_token.text])
                continue
