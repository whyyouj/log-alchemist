...
def graph_agent(state: list):
    print("graph agent")
    prompt = state['input']
    llm = Graph_Ai(model = 'llama3.1', df=DF)
    out = llm.run(prompt=prompt)
    if out != "No code available":
        return {"agent_out" : state['agent_out'], 'graph_out': out}
    else:
        return {"agent_out":state["agent_out"], 'graph_out' : ''}
...
def python_pandas_ai(state:list):
    ...
    prompt = state['input']
    llm = Python_Ai(model = 'llama3.1', df = DF)
    out = llm.pandas_ai_agent(prompt)
    return {"agent_out": out}
